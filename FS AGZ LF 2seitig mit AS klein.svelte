{#each schueler as s (s.ID)}
  {#each s.abschnitte.filter(aktHalbjahr) as hj (hj.ID)}
    <div class="page grid" orientation="portrait" size="A4" style="font-size: 80%;">
      <Pageheader art="daten/agz.svg" logo="{privat.logo}" untertitel="{privat.untertitel}" traeger="{privat.traeger}"/>
      <div class="main">
        <span style="font-size: 1.2rem; line-height: 1.2em">
          {bg(s, 'Schulform')} |
        </span>
        <span style="font-size: 1rem; line-height: 1.0em">
          {bg(s, 'Bereich')}
        </span>
        <Voffset v=".5"/>
